    ...
    <>
      {loadError && (
        <div className="bg-danger-bg text-danger text-[13px] rounded-[9px] px-3 py-2 mb-4">
          Couldn&apos;t load teachers: {loadError}
        </div>
      )}
      ...
      <div className="bg-surface border border-border rounded-card shadow-sm overflow-hidden">
        <table className="w-full text-[13px] border-collapse">
          ...
          <tbody>
            {teachers.length === 0 && (
              <tr><td colSpan={6} className="text-center text-muted py-10">No teachers yet.</td></tr>
            )}
            {teachers.map(t => (
              <tr key={t.id}>
        ...
        <div className="fixed inset-0 bg-primary-dark/35 z-50 flex justify-end" onClick={() => setShowAdd(false)}>
          <div className="w-[460px] max-w-[92vw] bg-surface h-full overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-border flex justify-between items-start sticky top-0 bg-surface">
              <h3 className="font-display text-[17px] font-semibold">Add Teacher</h3>
            ...
            <form action={handleAdd} className="px-6 py-[22px] flex flex-col gap-4">
              ...
              <F label="Login Email" name="email" type="email" required />
              <F label="Temporary Password" name="password" type="text" required />
              <F label="Subject" name="subject" placeholder="Hifz / Nazira / Tajweed" />
              <F label="Monthly Salary" name="monthly_salary" type="number" />
              ...
              <p className="text-[11.5px] text-muted -mt-2">Share the email + temporary password with the teacher so they can log in.</p>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-primary-dark/35 z-50 flex items-center justify-center" onClick={() => setEditTarget(null)}>
          <div className="w-[400px] max-w-[92vw] bg-surface rounded-card p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-display text-[16px] font-semibold">Edit Teacher</h3>
            ...
            <form action={handleEditSave} className="flex flex-col gap-4">
              ...
              <div>
                <label className="block text-[11.5px] font-semibold text-muted uppercase tracking-wide mb-[5px]">Subject</label>
              ...
              <div>
                <label className="block text-[11.5px] font-semibold text-muted uppercase tracking-wide mb-[5px]">Monthly Salary</label>
