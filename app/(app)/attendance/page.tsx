...
export default async function AttendancePage() {
  ...
  let studentQuery = supabase.from('students').select('id, full_name, classes(name)').eq('status', 'Active')
  ...
  const { data: todayAttendance } = await supabase.from('attendance').select('student_id, teacher_id, status').eq('date', today)
