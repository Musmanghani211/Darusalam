...
// student with a monthly fee set — one for each month (from their
// fee_effective_from date, falling back to admission_date, up to the
// current month) that doesn't already have a real row in the `fees`
// table. Used by both the Fees page and the Dashboard so their numbers
// always match.
export function generateVirtualFees(students: StudentForFees[], realFees: RealFee[]) {
  ...
  for (const s of students) {
    ...
    const startDate = s.fee_effective_from || s.admission_date
    const months = monthLabelsSince(startDate)
